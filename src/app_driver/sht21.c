...
STATIC UCHAR_T __sht21_check_crc(UCHAR_T data[], UCHAR_T byte_num, UCHAR_T check_sum)
{
    ...
    if(crc != check_sum) {
        PR_NOTICE("SHT21 check crc error!");
        return -1;
    } else {
        return 0;
    } 
...
{   
    ...
    PR_NOTICE("error = %d",error);
    __sht21_stop();

    measurand |= (data[0]<<8);
    measurand |= data[1];
    
    PR_NOTICE("measurand = %d",measurand);
...
UCHAR_T tuya_sht21_init(sht21_init_t* sht21_init_param)
{   
    UCHAR_T error = 0;
    ...
    i2c_pin_t i2c_config = {
        .ucSDA_IO = sht21_init_param ->SDA_PIN,
        .ucSCL_IO = sht21_init_param ->SCL_PIN,
    };
    opRet = opSocI2CInit(&i2c_config);          /* SDA&SCL GPIO INIT */
    PR_NOTICE("SocI2CInit = %d",opRet);
    
    error = tuya_sht21_read_register(&user_register);
    user_register = (user_register & ~SHT2x_RES_MASK) | sht21_init_param->RESOLUTION;
